...
class ImagePredictor {
    /// - Tag: name
    static func createImageClassifier(modelType:Int) -> VNCoreMLModel {
        // Use a default model configuration.
        let defaultConfig = MLModelConfiguration()
        
        let logisticWrapper = try? logistic_model(configuration: defaultConfig)
        
        let boostedTreeWrapper = try? boosted_model(configuration: defaultConfig)

        guard let logisticClassifier = logisticWrapper else {
            fatalError("App failed to create an logistic model instance.")
        }
        guard let boostedTreeClassifier = boostedTreeWrapper else {
            fatalError("App failed to create an boostedTree model instance.")
        }

        // Get the underlying model instance.
        let imageClassifierModel = modelType == 0 ? logisticClassifier.model : boostedTreeClassifier.model
    ...
    /// - Tag: makePredictions
    func makePredictions(for photo: UIImage,modelType:Int, completionHandler: @escaping ImagePredictionHandler) throws {
        ...
        let imageClassificationRequest = VNCoreMLRequest(model: ImagePredictor.createImageClassifier(modelType: modelType),
