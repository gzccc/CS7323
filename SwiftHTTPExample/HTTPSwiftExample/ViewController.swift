...
let SERVER_URL = "http://192.168.1.218:8000" // change this for your server name!!!
...
class ViewController: UIViewController, URLSessionDelegate {
        ...
        let dataTask : URLSessionDataTask = self.session.dataTask(with: request,
            completionHandler:{(data, response, error) in
                if(error != nil){
                    ...
                    let dsid = jsonDictionary["data"] as! [NSDictionary]
                    
                    if dsid.count > 0{
                       
                        print(dsid)
                        DispatchQueue.main.async{
                            let vc = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "SelectDataViewController") as! SelectDataViewController
                            vc.dsidArray = dsid as! [NSDictionary]
                            vc.selectDsidReturn = { (selectDsid) in
                                 self.dsid = selectDsid
                            }
                            self.present(vc, animated: true)
                        }
                    }else{
                        
                        self.alertTipsWith(msg: "have not data, add frist")
                    }
                }
    ...
    @IBAction func startCalibration(_ sender: AnyObject) {
        ...
        if self.dsid == "None"{
            ...
            alertTipsWith(msg: "select DSID frist")
            return
        }
